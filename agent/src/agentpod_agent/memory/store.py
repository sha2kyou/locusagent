...
_MEMORY_TERMS = frozenset({"long_term", "short_term"})


def resolve_memory_anchor_input(value: str | None, *, default: str = "short_term") -> str:
    """Map memory tool term (long_term / short_term) to stored anchor (identity / experience)."""
    raw = str(value or default or "short_term").strip().lower()
    if raw == "long_term":
        return MEMORY_ANCHOR_LONG
    if raw == "short_term":
        return MEMORY_ANCHOR_SHORT
    raise ValueError("term must be long_term or short_term")


def memory_term_label(anchor: str | None) -> str:
    return "long-term" if _normalize_anchor(anchor) == MEMORY_ANCHOR_LONG else "short-term"
