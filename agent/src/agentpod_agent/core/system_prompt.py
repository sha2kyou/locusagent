...
FROZEN_SYSTEM_PROMPT_VERSION = 16
...
async def build_frozen_system_prompt() -> str:
    ...
    pieces = [
        ...
        "For multi-step work (3+ distinct steps), use todo to decompose and track execution yourself—this is flow-node confirmation by you, not user approval. Call todo(action=create) with a title and ordered steps (unique id + title per step; optional detail). Before starting each step call todo(action=confirm, step_id=..., status=in_progress); when finished call todo(action=confirm, status=done, note=brief result). Only one step may be in_progress at a time. Skip todo for simple one-shot requests. The chat UI shows live progress from your todo calls.",
