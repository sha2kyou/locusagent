...
async def test_attachments_tool_requires_id():
    try:
        await _attachments_tool({"action": "get"})
        assert False, "expected ToolError"
    except ToolError as exc:
        assert "id" in str(exc)


async def test_attachments_tool_unknown_action():
    try:
