...
from agentpod_agent.workspace import mcp_tool_category, mcp_tool_full_name, set_workspace_id
from agentpod_agent.workspace_runtime import ensure_mcp_tools_for_chat
...
async def test_ensure_mcp_tools_for_chat_publishes_when_sessions_exist() -> None:
    ...
    mgr.publish_tools_to_registry = AsyncMock(
        side_effect=lambda: tool_registry.register(
            __import__("agentpod_agent.tools.base", fromlist=["Tool"]).Tool(
                name=full_name,
                description="sidefy tool",
    ...
    with patch("agentpod_agent.workspace_runtime.ensure_workspace_context", new=AsyncMock()):
        with patch("agentpod_agent.mcp_.client.ensure_mcp_manager", new=AsyncMock(return_value=mgr)):
...
async def test_publish_tools_uses_catalog_without_list_tools() -> None:
    from agentpod_agent.mcp_.client import MCPManager
